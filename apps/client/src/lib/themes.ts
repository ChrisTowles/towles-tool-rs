// Theme data ported verbatim from slot-1
// `packages/agentboard/src/runtime/themes.ts`. The vestigial per-theme `icons`
// map is dropped (UI-SPEC §4/§6 — components use the fixed glyph set instead);
// only `palette` (21 keys) + `status` (per-AgentStatus color) are carried.

import type { AgentStatus } from "../types";

/** The 21 Catppuccin-style palette keys used by every theme. */
...
/** The ordered list of palette keys (drives the CSS-variable mapping). */
...
/**
 * Resolve a theme from config. Ported verbatim from slot-1 `resolveTheme`.
 * undefined→default; string→builtin ?? default; partial→shallow-merge over
 * default.
 */
