//! Self-registers a `.desktop` entry + icon per task so each checkout shows up
//! as its own entry in the GNOME/COSMIC app launcher/search, with the right
//! icon and an `Exec` pointing at that task's own binary. `tauri build`
...
//! actually differs, so every task's binary can call this on startup without
...
//! worktree's window a D-Bus-activatable singleton, and any activation
...
//! find *an* icon for the running window, just not a per-task one.
