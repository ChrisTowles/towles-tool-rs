...
Once a variant has won, write down which one and why (commit message, a GitHub `decision` issue, or a `NOTES.md` next to the prototype if running AFK and the user hasn't responded yet — we don't write in-repo ADRs). Then:
