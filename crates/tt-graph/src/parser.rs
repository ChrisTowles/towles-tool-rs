//! Day-based filtering of session results. Ports the filtering half of
//! `src/commands/graph/parser.ts`.
//!
//! Transcript **parsing** (`parse_transcript`, token counting, title extraction)
//! now lives in the shared [`tt_claude_code`] crate — the single home for the
//! internal Claude Code JSONL schema. This module keeps only tt-graph's own
//! mtime-based day filter, whose clock is injected (`now_ms`) so tests stay
//! deterministic.
