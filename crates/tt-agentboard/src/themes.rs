...
impl Theme {
    pub fn status_color(&self, status: AgentStatus) -> &'static str {
        match status {
            ...
            AgentStatus::Busy => self.status.busy,
            AgentStatus::Complete => self.status.complete,
...
mod tests {
    ...
    fn spot_check_against_ts_source() {
        ...
        assert_eq!(latte.status.busy, "#df8e1d");
    ...
    fn status_colors_and_icons_cover_every_status() {
        ...
        assert_eq!(status_icon(AgentStatus::Complete), "✓");
    }
}
