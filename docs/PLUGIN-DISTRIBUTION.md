...
`tt@towles-tool` (plugin `tt`, marketplace `towles-tool`). Note `ttr install` also
...
- **Existing installs:** unchanged. Everyone stays on the live repo's marketplace;
  `claude plugin update` keeps pulling from it. `ttr install`'s URL already matches.
...
- **Migration steps:** add a git remote for this repo, publish it, add a root
  `.claude-plugin/marketplace.json` here, and update `ttr install` + docs to the new
...
- **Migration steps:** create a standalone repo holding only `packages/core` +
  `marketplace.json`, publish it, repoint `ttr install`/docs. This repo would drop
...
- Whatever is chosen must keep `ttr install`'s marketplace URL and the published
